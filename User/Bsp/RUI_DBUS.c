...
void RUI_F_DUBS_Resovled(uint8_t* Data, DBUS_Typedef *RUI_V_DBUS)
{
    RUI_V_DBUS->RUI_V_DBUS_ONLINE_JUDGE_TIME = RUI_DF_DBUS_OFFLINE_TIME;
		RUI_V_DBUS->Remote.S1_Last = RUI_V_DBUS->Remote.S1_u8;
		RUI_V_DBUS->Remote.S2_Last = RUI_V_DBUS->Remote.S2_u8;
...
void F_DUBS_CONTROL(DBUS_Typedef *vDbus , CONTAL_Typedef *V_CONTAL)
{
	...
	switch (vDbus->Remote.S2_u8 * 3 + vDbus->Remote.S1_u8)
	{	
/***************************************头部begin**************************************************/				
						//		S2	 + 	S1
		case 0x04:	    //		1		1:底盘
		{ 
			V_CONTAL->control[0].BOTTOM.VW = (float)(vDbus->Remote.CH2_int16) * 0.6f;
			V_CONTAL->control[0].BOTTOM.VX = (float)(vDbus->Remote.CH0_int16<<2);
			V_CONTAL->control[0].BOTTOM.VY = (float)(vDbus->Remote.CH1_int16<<1);
			
		}
        break;
		case 0x06:	    //		1		3:登岛+图传抬升
		{			
			V_CONTAL->control[0].STEPS.Rock_Arm += -((((float)(vDbus->Remote.CH1_int16))*PI/180.0)*0.001);
			V_CONTAL->control[0].STEPS.Track = ((float)vDbus->Remote.CH3_int16)*19.0*469.0/660.0;
			V_CONTAL->control[0].STEPS.Lead_Screw += (int32_t)vDbus->Remote.CH2_int16 ;
...
/***************************************中部begin**************************************************/		
		...
		case 0x0a:		//		3		1机械臂
		{
			V_CONTAL->control[0].MECHANICAL_ARM.J1_8009p += ((float)(vDbus->Remote.CH0_int16))*-0.00005;
			V_CONTAL->control[0].MECHANICAL_ARM.J2_8009P += ((float)(vDbus->Remote.CH1_int16))*0.00005;
			V_CONTAL->control[0].MECHANICAL_ARM.J3_4340 += ((float)(vDbus->Remote.CH2_int16))*0.00005;
			V_CONTAL->control[0].MECHANICAL_ARM.J4_4340 += ((float)(vDbus->Remote.CH3_int16))*0.00005;
		...
		case 0x0c:		//		3		3机械臂
		{
			V_CONTAL->control[0].MECHANICAL_ARM.J5_4310 += ((float)(vDbus->Remote.CH0_int16))*0.00005;
			V_CONTAL->control[0].MECHANICAL_ARM.J6_4310 += ((float)(vDbus->Remote.CH1_int16))*0.00005;
		}	
        break;
...
/***************************************底部begin**************************************************/
		...
		case 0x08:		//		2		2		键鼠模式
		{
			...
			//kernel_run(&WHW_V_DBUS, &ALL_CONTROL[0]);
